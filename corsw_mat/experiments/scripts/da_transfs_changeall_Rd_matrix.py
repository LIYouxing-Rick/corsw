...
from spdsw.spdsw_proj import SPDSW
...
parser.add_argument("--distance", type=str, default="olm", help="distance metric")
...
def olm_compress_zero_diagonal_matrix_5d(S):
    """
    将对角线为0的对称矩阵从(bs,1,1,d,d)压缩到(bs,1,1,d-1,d-1)
    """
    bs, _, _, d, _ = S.shape
    device = S.device
    dtype = S.dtype
    
    S_3d = S.squeeze(1).squeeze(1)
    
    diagonal = torch.diagonal(S_3d, dim1=-2, dim2=-1)
    if not torch.allclose(diagonal, torch.zeros_like(diagonal), atol=1e-6):
        S_3d = S_3d.clone()
        S_3d.diagonal(dim1=-2, dim2=-1).fill_(0)
    
    S_3d = (S_3d + S_3d.transpose(-2, -1)) / 2
    
    tril_indices = torch.tril_indices(d, d, offset=-1, device=device)
    lower_elements = S_3d[:, tril_indices[0], tril_indices[1]]
    
    d_compressed = d - 1
    S_compressed_3d = torch.zeros((bs, d_compressed, d_compressed), dtype=dtype, device=device)
    
    compressed_tril_indices = torch.tril_indices(d_compressed, d_compressed, offset=0, device=device)
    n_positions = len(compressed_tril_indices[0])
    
    if lower_elements.shape[1] >= n_positions:
        S_compressed_3d[:, compressed_tril_indices[0], compressed_tril_indices[1]] = lower_elements[:, :n_positions]
    else:
        available_elements = lower_elements.shape[1]
        S_compressed_3d[:, compressed_tril_indices[0][:available_elements], 
                        compressed_tril_indices[1][:available_elements]] = lower_elements
    
    S_compressed_3d = S_compressed_3d + S_compressed_3d.transpose(-2, -1)
    
    diagonal_mask = torch.eye(d_compressed, device=device, dtype=dtype).bool()
    S_compressed_3d[:, diagonal_mask] = S_compressed_3d[:, diagonal_mask] / 2
    
    S_compressed = S_compressed_3d.unsqueeze(1).unsqueeze(1)
...
def run_test(params):
    ...
    if cross_subject:
        ...
        cor_Xs = cov2corr(covp_Xs)
        cor_Xs = correlation.symmetrize(cor_Xs)
        L_Xs = manifold.deformation(cor_Xs)
        sym_Xs = olm_compress_zero_diagonal_matrix_5d(L_Xs)
        
        L_Xs1 = manifold1.deformation(cor_Xs)
        sym_Xs1 = olm_compress_zero_diagonal_matrix_5d(L_Xs1)
        ...
        cor_Xt = cov2corr(covp_Xt)
        cor_Xt = correlation.symmetrize(cor_Xt)
        L_Xt = manifold.deformation(cor_Xt)
        sym_Xt = olm_compress_zero_diagonal_matrix_5d(L_Xt)
        
        L_Xt1 = manifold1.deformation(cor_Xt)
        sym_Xt1 = olm_compress_zero_diagonal_matrix_5d(L_Xt1)
        ...
        L_Xs2 = manifold.deformation(cor_Xs)
        sym_Xs2 = olm_compress_zero_diagonal_matrix_5d(L_Xs2)
        ...
        L_Xt2 = manifold.deformation(cor_Xt)
        sym_Xt2 = olm_compress_zero_diagonal_matrix_5d(L_Xt2)
        
        yt = torch.tensor(yt, device=DEVICE, dtype=torch.int) - 1

    n_freq = cov_Xs.shape[2]
    model = Transformations(d-1, n_freq, DEVICE, DTYPE, seed=seed)
    ...
    s_noalign = get_svc(
        sym_Xs1[:, 0].detach().cpu(),
        sym_Xt1[:, 0].detach().cpu(),
        ys.detach().cpu(),
        yt.detach().cpu(),
        d-1, multifreq, n_jobs=N_JOBS, random_state=seed
    )
    s_align = get_svc(
        model(sym_Xs1)[:, 0].detach().cpu(),
        sym_Xt1[:, 0].detach().cpu(),
        ys.detach().cpu(),
        yt.detach().cpu(),
        d-1, multifreq, n_jobs=N_JOBS, random_state=seed
