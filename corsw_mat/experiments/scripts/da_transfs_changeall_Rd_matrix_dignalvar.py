...
def run_test(params):
    ...
    if cross_subject:
        ...
        L_Xs = manifold.deformation(cor_Xs)
        diag_elements_Xs = torch.diagonal(cov_Xs, dim1=-2, dim2=-1)
        diag_matrix_Xs = torch.diag_embed(diag_elements_Xs)
        
        L_Xs = linalg.sym_logm(diag_matrix_Xs) + L_Xs
        ...
        L_Xt = manifold.deformation(cor_Xt)
        diag_elements_Xt = torch.diagonal(cov_Xt, dim1=-2, dim2=-1)
        diag_matrix_Xt = torch.diag_embed(diag_elements_Xt)
        
        L_Xt = linalg.sym_logm(diag_matrix_Xs) + L_Xt
