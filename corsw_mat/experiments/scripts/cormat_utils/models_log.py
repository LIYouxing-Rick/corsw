...
def get_svc(sym_Xs, sym_Xt, ys, yt, d, multifreq=False, n_jobs=50, random_state=None, kernel=False):
    sym_Xs = sym_Xs.detach().cpu() #.reshape(-1, d * d)
    sym_Xt = sym_Xt.detach().cpu() #.reshape(-1, d * d)
    
    if multifreq:
        sym_Xs = sym_Xs.numpy()
        sym_Xt = sym_Xt.numpy()
    if not multifreq:
        sym_Xs = sym_Xs.reshape(-1, d*d)
        sym_Xt = sym_Xt.reshape(-1, d*d)
...
#         )
    ...
    clf.fit(sym_Xs, ys.cpu())
    return clf.score(sym_Xt, yt.cpu())