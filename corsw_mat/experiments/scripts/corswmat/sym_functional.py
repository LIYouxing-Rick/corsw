...
class sym_modeig:
    ...
    def forward(M : Tensor, fun : Callable[[Tensor], Tensor], fun_param : Tensor = None,
                ensure_symmetric : bool = False, ensure_psd : bool = False, mode='svd') -> Tensor:
...
class sym_expm(Function):
    ...
    @staticmethod
    def forward(ctx: Any, M: Tensor, ensure_symmetric : bool = False) -> Tensor:
        X, s, smod, U = sym_modeig.forward(M, sym_expm.value, ensure_symmetric=ensure_symmetric,mode='eigh')
        ctx.save_for_backward(s, smod, U)
        return X
...
class sym_powm(Function):
    ...
    def value(s : Tensor, exponent : Tensor) -> Tensor:
        if isinstance(exponent, Tensor) and exponent.numel() > 1:
            return s.pow(exponent=exponent.squeeze().unsqueeze(-1)) # for channel-wise LieBN
        else:
    ...
    def derivative(s : Tensor, exponent : Tensor) -> Tensor:
        if isinstance(exponent, Tensor) and exponent.numel() > 1:
            return exponent.squeeze().unsqueeze(-1) * s.pow(exponent=(exponent-1.).squeeze().unsqueeze(-1))  # for channel-wise LieBN
        else:
    ...
    def backward(ctx: Any, dX: Tensor):
        ...
        ## should deal with channel and batch dim
...
class sym_Glogm(Function):
    ...
    def backward(ctx: Any, dX: Tensor):
        ...
        if s.dim()==1:
            ...
            dw = tmp.diagonal(dim1=-2, dim2=-1).mul(sigma_log).sum(dim=tuple(range(tmp.ndim - 2)))
        return dz, dw
...
class sym_Gexpm(Function):
    ...
    @staticmethod
    def forward(ctx: Any, M: Tensor, weight: Tensor, ensure_symmetric : bool = False) -> Tensor:
        X, s, smod, U = sym_modeig.forward(M, sym_Gexpm.value, ensure_symmetric=ensure_symmetric,mode='eigh',fun_param=weight)
        ctx.save_for_backward(s, smod, U, weight)
        return X
    ...
    def backward(ctx: Any, dX: Tensor):
        ...
        tmp = U.transpose(-1, -2).matmul(dX).matmul(U)
        # here should be summation, even for batch dim
        if s.dim()==1:
            dw = tmp.diagonal(dim1=-2, dim2=-1).mul(internal)
        else:
            dw = tmp.diagonal(dim1=-2, dim2=-1).mul(internal).sum(dim=tuple(range(tmp.ndim - 2)))
        return dz,dw
