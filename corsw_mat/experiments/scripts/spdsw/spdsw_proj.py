...
def phi_star_0_lsm_efficient(v):
    """
    更高效的实现，避免创建完整的对角矩阵
    
    Args:
        v: torch.Tensor, shape (n_proj, d, d) - 输入矩阵批量
    
    Returns:
        result: torch.Tensor, shape (n_proj, d, d) - 变换后的矩阵
    """
    # 计算 v²
    v_squared = torch.bmm(v, v)
    
    # 计算行和
    row_sums = v.sum(dim=-1)  # (n_proj, d)
    
    # 直接从v²的对角线减去行和
    result = v_squared.clone()
    diag_indices = torch.arange(v.shape[1], device=v.device)
    result[:, diag_indices, diag_indices] -= row_sums
...
class SPDSW:
    ...
    def generate_projections(self, shape_X, num_projections, num_ts,
        ...
        theta = rng.normal(size=(num_projections, shape_X-1))
        theta = F.normalize(
        ...
        Z = rng.normal(size=(num_projections, shape_X-1, shape_X-1))
        Z = torch.tensor(
            Z,
            dtype=dtype,
            device=device
        ...
        self.A =phi_star_0_lsm_efficient(v)
